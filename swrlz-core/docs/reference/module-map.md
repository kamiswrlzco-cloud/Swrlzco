> **Migration note:** historical `.reference` entries remain lineage evidence. Current promoted source authority is CLIENT CFv2.1.9 / SERVER CFv2.1.0; see `../CURRENT_AUTHORITY.md`. SERVER CFv2.1.9 R1-R6 and CLIENT CFv2.1.22 R1 entries below are candidate-module evidence, not promotion.
...
| `CLIENT CFv2.1.22 R1 candidate transport` | CLIENT Update Ledger / Settings / Theme identity candidate | SWURLZER | Candidate UX/settings/theme-identity progression with manifest-backed source/static evidence | SOURCE CANDIDATE | `docs/client/CLIENT_IMPLEMENTATION_ANALYSIS.md`; `docs/checkpoints/INT-DOC-AI-041H_DENSE_CHAT_IDENTITY_MODEL_SYNC.md` |
...
| `SERVER CFv2.1.9 R6: app/src/main/java/sh/swrlz/nodehost/ai/ServerProviderMeshRuntime.kt` | SERVER Provider Mesh Runtime | SWURVER + SWRLZ CORE | Routes advisory provider reasoning while preserving SWRLZ control-plane policy | SOURCE CANDIDATE | `docs/checkpoints/INT-DOC-AI-041H_DENSE_CHAT_IDENTITY_MODEL_SYNC.md` |
| `SERVER CFv2.1.9 R6: app/src/main/java/sh/swrlz/nodehost/ai/local/SwrlieLocalRuntime.kt` | SWRLIE Local Runtime | SWURVER + MODEL | Local model load/inference orchestration behind SWRLIE provider interface | SOURCE CANDIDATE | `docs/architecture/SWRLZ_IDENTITY_PROFILE_AND_REASONING_EQUIPMENT_V1.md` |
...
| `SERVER CFv2.1.9 R6: app/src/main/java/sh/swrlz/nodehost/ai/local/SwrliePromptBudgeter.kt` | Prompt Budget Guard | SWRLZ CORE + MODEL | Reserves runtime/output budget before native inference and bounds compiled identity/context | SOURCE CANDIDATE | `docs/architecture/SWRLZ_SWRLIE_RUNTIME_CAPABILITY_AND_SKILLS_EVOLUTION_V1.md` |
| `SERVER CFv2.1.9 R6: app/src/main/java/sh/swrlz/nodehost/ai/local/SwrlieRuntimeSettings.kt` | SWRLIE Runtime Settings | SWURVER + MODEL | Presets/context/output/sampling/thread/keep-loaded resource configuration | SOURCE CANDIDATE | `docs/architecture/SWRLZ_SWRLIE_RUNTIME_CAPABILITY_AND_SKILLS_EVOLUTION_V1.md` |
| `SERVER CFv2.1.9 R6: app/src/main/java/sh/swrlz/nodehost/ai/context/ServerSelfKnowledgeResolver.kt` | Local Self-Knowledge Resolver | SWRLZ CORE + SWURVER | Supplies installed identity, role map, patch/runtime/model state to local SWRLIE without granting authority | SOURCE CANDIDATE | `docs/checkpoints/INT-DOC-AI-041H_DENSE_CHAT_IDENTITY_MODEL_SYNC.md` |
| `SERVER CFv2.1.9 R6: app/src/main/java/sh/swrlz/nodehost/update/ServerUpdateLedgerRepository.kt` | Update Ledger Repository | SWURVER | Shared packaged update-history parser/retrieval source for UI and local self-knowledge | SOURCE CANDIDATE | `docs/checkpoints/INT-DOC-AI-040B-R1-R5_SWRLIE_RUNTIME_SYNC.md` |
| `SERVER CFv2.1.9 R6: app/src/main/java/sh/swrlz/nodehost/ui/ServerChatScreen.kt` | SERVER Chat + Machine Status Stack | SWURVER | First-class Chat UI plus R6 tappable status portals over authoritative SERVER/model/node/network/health state | SOURCE CANDIDATE | `docs/server/SERVER_IMPLEMENTATION_ANALYSIS.md`; `docs/checkpoints/INT-DOC-AI-041H_DENSE_CHAT_IDENTITY_MODEL_SYNC.md` |
| `SERVER CFv2.1.9 R6: app/src/main/java/sh/swrlz/nodehost/ui/ServerUpdateLedgerDialog.kt` | Update Ledger UI | SWURVER | Horseshoe-opened local revision/patch-note surface consuming shared repository | SOURCE CANDIDATE | `docs/checkpoints/INT-DOC-AI-040B-R1-R5_SWRLIE_RUNTIME_SYNC.md` |
| `docs/architecture/SWRLZ_IDENTITY_PROFILE_AND_REASONING_EQUIPMENT_V1.md` | Identity/Profile/Reasoning contract | SWRLZ CORE | Defines SWRLZ primary identity, replaceable LLM/model role, Swurlzara lens role, intrinsic Truth Firewall and swap invariants | DOCUMENTED | same file |
...
Notes:
- Candidate path entries use the source ZIP's internal path because candidates are stored in repository transport form rather than extracted into the maintained Git tree.
- The R6 repository transport proves source identity/checksum but does not include the separately packaged candidate manifest.
