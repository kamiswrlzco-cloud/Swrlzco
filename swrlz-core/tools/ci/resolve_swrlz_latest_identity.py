...
def _exists(repo_root: Path, ref: str, path: PurePosixPath) -> bool:
    # Resolve existence from the commit tree only. Blob inspection can
    # trigger lazy materialization in a blob:none partial clone, so the
    # prepare job must prove path existence using tree metadata instead.
    result = subprocess.run(
...
def _identity_source_name(name: str) -> str:
    if name.lower().endswith(".transport.json"):
        return name[: -len(".transport.json")] + ".zip"
    return canonical_zip(name)


def _identity_rank_hint(name: str) -> tuple:
    source_name = _identity_source_name(name)
    return (
        version_tuple(source_name),
        revision_number(source_name),
        name.casefold(),
...
def resolve_latest_identity(repo_root: Path, component: str, *, ref: str = "HEAD") -> str:
    ...
    identities.sort(
        key=lambda identity: _identity_rank_hint(identity.name),
        reverse=True,
    )

    for identity in identities:
        try:
            if identity.name.upper().endswith(".TRANSPORT.JSON"):
                _, resolved = _transport_candidate(
                    repo_root, ref, lane, identity, entries, component
                )
            else:
                _, resolved = _direct_candidate(
                    repo_root, ref, lane, identity, entries, component
                )
            return resolved
    ...
    raise LatestIdentityError(
        f"no verified {component} source identity found at {ref}"
    )
